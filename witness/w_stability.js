// W-ROOM-OCCL-STABILITY — issue proved/disproved: does the ROOM_STABLE_N membership-stability
// gate stop A→B→A flap sequences (§11.2 Q4 measured 17 such flaps returning within ≤10 frames)
// from triggering a room-state change? Synthetic flap: camera alternates between two rooms every
// FLAP_HOLD(5) frames — faster than any measured flap — for many cycles; roomChanges must not
// move. Then a genuine hold ≥ ROOM_STABLE_N frames must be accepted (gate ≠ deadlock).
const fs = require('fs');
...
(async () => {
  ...
  try {
    ...
    const pass = p1.roomCur === res.roomA && p2.changesDuringFlap === 0 && p2.roomCur === res.roomA &&
      p3.roomCur === res.roomB;
    sink('§ROOM_OCCL_STABILITY holdA_frames=' + p1.frames + ' roomA=' + res.roomA +
      ' flap5x24_changes=' + p2.changesDuringFlap + ' flap_roomCur=' + p2.roomCur +
      ' holdB_frames=' + p3.frames + ' roomB=' + res.roomB +
      ' verdict=' + (pass ? 'PASS' : 'FAIL'));
  } finally {
    fs.writeFileSync(__dirname + '/w_stability.log', LOG.join('\n'));
    await browser.close();
  }
})().catch(e => { LOG.push('ERR ' + e.stack); fs.writeFileSync(__dirname + '/w_stability.log', LOG.join('\n')); process.exit(1); });
