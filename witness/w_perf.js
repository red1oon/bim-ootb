// W-ROOM-OCCL-PERF (the actual deliverable) — issue proved/disproved: does room-mismatch demote
// measurably improve interior flyPath legs on LTU, on real GPU, vs shipped distance/frustum-only
// DLOD-nav? Same machine, same session, same route (graph tour is deterministic per building).
// Four tour runs interleaved false,true,true,false to cancel warm-up drift; per-frame rAF dt +
// renderer.info.render.calls sampled on INTERIOR (flyPath) frames only. A negative/no-win result
// is a valid outcome (§13 non-goals) — reported plainly either way.
const fs = require('fs');
...
async function runTour(page, enabled, runName) {
  await page.evaluate((en) => {
    ...
    window.__dlodNav.roomOcclEnabled = en;
  ...
  const roomState = await page.evaluate(() => ({
    ...
    rects: window.__dlodNav.roomIdxRects }));
  fs.writeFileSync(__dirname + '/w_perf_' + runName + '.json', JSON.stringify(samples));
  const st = stats(samples.dts), cs = stats(samples.calls), bx = stats(samples.boxed);
  sink('§ROOM_OCCL_PERF run=' + runName + ' enabled=' + enabled +
    ...
    ' roomChanges=' + roomState.changes + ' routeSigLen=' + routeSig.length);
...
(async () => {
  ...
  try {
    ...
    sink('§ROOM_OCCL_PERF_SUMMARY sameRoute=' + sameRoute +
      ' OFF frame_ms=' + off.ms.toFixed(2) + ' p95=' + off.p95.toFixed(2) + ' drawCalls=' + off.calls.toFixed(0) + ' boxed=' + off.boxed.toFixed(0) +
      ' | ON frame_ms=' + on.ms.toFixed(2) + ' p95=' + on.p95.toFixed(2) + ' drawCalls=' + on.calls.toFixed(0) + ' boxed=' + on.boxed.toFixed(0) +
      ' | delta_ms=' + (on.ms - off.ms).toFixed(2) + ' delta_calls=' + (on.calls - off.calls).toFixed(0));
  } finally {
    fs.writeFileSync(__dirname + '/w_perf.log', LOG.join('\n'));
    await browser.close();
  }
})().catch(e => { LOG.push('ERR ' + e.stack); fs.writeFileSync(__dirname + '/w_perf.log', LOG.join('\n')); process.exit(1); });
