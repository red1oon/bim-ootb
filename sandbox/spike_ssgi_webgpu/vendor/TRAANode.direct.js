// PATCHED COPY (2026-09-22, sandbox spike): only change from ../TRAANode.js is the two 'three/webgpu'/'three/tsl' bare-specifier imports rewritten to direct relative paths ('./r186/three.webgpu.js' / './r186/three.tsl.js') so this file loads with NO import map — needed because injecting one via document.write from an evaluateOnNewDocument tap script hung page navigation (see run_dual_gpu_test.js). Everything else byte-identical to the original vendored file.
import { HalfFloatType, Vector2, RenderTarget, RendererUtils, QuadMesh, NodeMaterial, TempNode, NodeUpdateType, Matrix4, DepthTexture, FloatType } from './r186/three.webgpu.js';
import { float, Fn, max, texture, uniform, uv, vec2, convertToTexture, passTexture, velocity, ivec2, mix, context, OnBeforeRenderPipeline, OnAfterRenderPipeline } from './r186/three.tsl.direct.js';
import { clipAABB, computeHaltonOffsets, flickerReduction, sampleCurrentDepth, samplePreviousDepth } from '../utils/TAAUtils.direct.js';
