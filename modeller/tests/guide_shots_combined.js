...
(async () => {
  ...
  // ── SHOT 1: Duplex ELEC schedule-walk, real LOD400 device meshes ─────────────────────────────
  {
    ...
    await xrayOn(pg, logs);   // glass ARC → the 102 LOD400 devices glow through in discipline colour
    await frame(pg, box, { dir: [1, -1, 0.55], dist: 12, tz: 0 });
    ...
    const clip1 = await projClip(pg, inHood1, 70);
    await pg.screenshot({ path: path.join(OUT, 'duplex_elec_lod400_walk.png'), clip: clip1, timeout: 15000 });
  ...
  // ── SHOT 2: SampleHouse rotated ELEC fixture — committed fold at correct yaw ─────────────────
  {
    ...
    await pg.screenshot({ path: path.join(OUT, 'samplehouse_elec_rotation_fix.png'), clip: clip2, timeout: 15000 });
