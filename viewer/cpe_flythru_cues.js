...
function setupCpeFlythruCues(A) {
  ...
  A.flythruCuesCompositeOntoCanvas = function (ctx, w, h, filmSec) {
    ...
    if (!a && _cues) {
      ...
      if (_env && filmSec > _env.at + SPAN && filmSec <= _env.at + SPAN + PANEL_HOLD) {
        var _tx = filmSec - (_env.at + SPAN);
        a = { cue: _env, panelOnly: true, opacity: _tx <= PANEL_HOLD - SLOT.fadeOut ? 1 : Math.max(0, (PANEL_HOLD - _tx) / SLOT.fadeOut) };
      }
    }
