...
// ~3x slower than canvas actually plays. Fixed: materializeZones forwards opts.shiftHours (undefined
// stays byte-identical, witnesses unaffected); the two real UI entry points now pass window.SHIFT_HOURS.
// Measured Hospital: old call span 88d (8h default) -> fixed call span 30d (24h), 2.93x. Does NOT
// touch kernel_ops/canvas — _GANTT_CACHE_VERSION unchanged. Bumping THIS version only so the browser
// actually fetches the fixed JS on next load (schedules/tasks tables are the user-editable product
// and are never auto-regenerated once materialized — see prompts/4D_SCHEDULE_PERFECTION.md).
