...
// v1156 (2026-09-06) §S274 FIX + §CPE_BAKE_RES_COMMAND (live crash reported from production): viewer/
//   cinema_path_editor.js — the Silent-bake size dropdown's change handler read `A.renderer` (A is the
//   app GETTER FUNCTION, not the object) and threw Cannot read properties of undefined (reading
//   'domElement') on every selection; fixed to call A(). Also fixed two stale "Alt+M" references
...
const CACHE_VERSION = 'v1274';   // bump on each deploy; per-change detail is the git commit message.
...
const SHELL_LIBS = [
  'lib/three.webgpu.min.js', // §GI_LIVE: r186 WebGPU (imports three.core.min.js) — was r184/r185 (§S276)
  'lib/three.module.min.js', // §S276: r184 standard ESM fallback
  'lib/three.core.min.js',  // §GI_LIVE: r186 core (split build); three.module.min.js (WebGL renderer) unchanged
...
const PRECACHE_ASSETS = [
  ...
  'surface_r10.js',
  'sky_portal.js',
  'sky_occlusion.js',   // §SKY_OCCLUSION: covered surfaces lose the open-sky hemi/env in Alt+S   // §SKY_PORTAL: Alt+S window panes as sky light sources
  'loader.js',
  'effects.js',
  'cinema_maxq.js',
  'gi_still.js',  // §GI_LIVE: Alt+S bounce (desktop + WebGPU only); its two lib/gi modules cache on first use
