...
const CACHE_VERSION = 'v632';   // v632: HISTORY KNOB-DIAL (HISTORY_KNOB_DIAL.md) — the line-art amp-knob replaces the bar's ↶/↷ model-undo buttons + the slider. NEW common/history_knob.js?v=1 (dot→bloom→6 ticks: 4 depth Off/Low/High*/Max + 2 nav back/front on the left flank; static-click; focus halo + ◄/► keys; detent sound mute-aware/haptic/debounced; reduced-motion; thumbnails deferred §LOCKED-5). The scrubber is now READ-ONLY: history_bar.js?v=3 adds a view-cursor (viewStepBack/Front/viewJumpTo + restoreView cfg) so dot clicks + knob nav re-apply a moment's stamped VIEW via HistoryTap.applyView — NEVER KernelOps.undo/redo (kills the "looks-corrupted" model mutation); MODEL undo/redo stays on Ctrl+Z/Backspace/toolbar. universal_history.js?v=12 supplies _restoreView. Witnesses common/tests/poc_knob.js 16/16 + viewer/tests/poc_knob_viewer.js 9/9 (dot+nav opLogMutated=NO, model undo still flips) + poc_cherry_pick PASS. PRIOR — v631: WHOLE-HISTORY cross-page TIME-scroller (HISTORY_WHOLE_TIMELINE.md W1–W3) — new common/whole_history.js?v=1 is the ONE cross-page log every surface mirrors into ({page,ts,label,kind,ref}); history_bar.js?v=2 `_mirror` now routes through it; universal_history.js?v=11 mounts the cross-page launcher (clock pill bottom-left → overlay: WHOLE|THIS toggle + day-axis time scroller, long-press-back steps DAY→DAY into a horizontally-scrollable STRETCH) + best-effort read-only restore. ADDITIVE — the viewer's own bottom bar is untouched. Witnesses tests/poc_whole_{log,bar,time}.js + *_dom.js all PASS. PRIOR — v630: pill reveal-UP cue — viewer #mobile-pill rises on open (PillBuilder _toggle `.pill-revealing` + @keyframes pill-rise in viewer.html); part of the cross-surface pill consistency pass (erp/sw.js v611). pill_builder.js?v=2. (supersedes v629 history-bar merge.)
