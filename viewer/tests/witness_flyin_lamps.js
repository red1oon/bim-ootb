  ...
  for (let k = 0; k < POSES.length; k++) { const ps = POSES[k];
    ...
    const r = await p.evaluate(() => { const A = window.APP, T = window.THREE, LZ = window.LightZones, lum = c => 0.2126 * c.r + 0.7152 * c.g + 0.0722 * c.b;
      ...
      for (let y = 0; y < 6; y++) for (let x = 0; x < 4; x++) { rc.setFromCamera(new T.Vector2(-1 + (x + 0.5) / 4 * 0.667, 1 - (y + 0.5) / 6 * 2), A.camera); const h = rc.intersectObjects(tg, false)[0]; if (!h || !h.face) continue;
        ...
        left.push({ z: si.zone, sky: si.sky, El: +El.toExponential(2), lampsReaching: reach, zoneLampsCouldReach: anyZoneLamp, kind: (+El > 0) ? 'lit' : (anyZoneLamp ? 'PICK_ZERO' : 'GEOMETRY_DARK'), cls: h.object.userData.ifcClass || '', gx: x, gy: y, n: [+N.x.toFixed(2), +N.y.toFixed(2), +N.z.toFixed(2)], distM: +h.distance.toFixed(1), nearestKeptM: +Math.sqrt(Math.min.apply(null, (A._nightLights || []).filter(l => l.visible && l.intensity > 0).map(l => l.position.distanceToSquared(h.point)))).toFixed(1) }); }
      const lz = {}; left.forEach(q => { lz[q.z] = lz[q.z] || { pts: 0, lampsKeptInZone: zones[q.z] ? zones[q.z].n : 0, El0: 0, pickZero: 0, geometryDark: 0 }; lz[q.z].pts++; if (!(+q.El > 0)) { lz[q.z].El0++; if (q.kind === 'PICK_ZERO') lz[q.z].pickZero++; else lz[q.z].geometryDark++; } });
    ...
    const expKept = Math.min(typeof r.cap === 'number' ? r.cap : 200, r.zoneLampsAvail), lw = r.leftZones[r.camZone] || { pts: 0, El0: 0, pickZero: 0, geometryDark: 0 };
    ...
    console.log('§LAMP_ZONE_PICK_GATE pose=' + (k + 1) + ' ' + (ok ? 'PASS' : 'FAIL') + ' camZone=' + r.camZone + ' camZoneKept=' + r.camZoneKept + ' expected=min(cap ' + r.cap + ', zoneLamps ' + r.zoneLampsAvail + ')=' + expKept +
      ' leftWallZero=' + lw.El0 + '/' + lw.pts + ' pickZero=' + lw.pickZero + ' geometryDark=' + lw.geometryDark + ' (lampRange ' + r.lampRange + ' m) decideMs=' + (dm.length ? dm.map(v => v.toFixed(1)).join('/') : 'n/a') + ' exposure=' + r.exposure + (k > 0 ? ' stopsFromPrev=' + (Math.log2(r.exposure / exposures[k - 1])).toFixed(2) : ''));
