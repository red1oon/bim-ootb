...
// FAULT (console.warn) when any counter > 0, else OK. Films pass through the same staging: the line prints there too.
(function (global) {
  ...
  function report(A) {
    ...
    var cam = A.camera, cp = cam.position, out = { unlit: 0, samples: 0, capDropNear: 0, extLightsDay: 0, glassLow: 0, portalsRetired: 0, expStep: 0, guard: guard.shaderError + guard.contextLost };
    ...
    var fault = out.unlit > 0 || out.capDropNear > 0 || out.extLightsDay > 0 || out.glassLow > 0 || out.guard > 0;
    var line = '§FAULT ' + (fault ? 'FAULT' : 'OK') + ' unlit=' + out.unlit + '/' + out.samples + ' capDropNear=' + out.capDropNear + ' extLightsDay=' + out.extLightsDay +
      (camOutside ? ' (camOutside' + (sunUp ? ', day)' : ', night)') : ' (camInside)') + ' glassLow=' + out.glassLow + ' portalsRetired=' + out.portalsRetired +
