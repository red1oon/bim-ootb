  ...
  A._doReportBug = function(mode, userDesc) {
    ...
    var _openIssue = function(logs) {
      ...
      if (mode === 'email') {
        ...
        var allLines = logs ? logs.split('\n') : [];
        var buildEmailUrl = function(n) {
          var body = [
            'Bug Report',
            '==========',
            '',
            ...
            envBlock,
            '',
            'Console Log (last ' + n + ' lines):',
            n ? allLines.slice(-n).join('\n') : '(omitted — mailto: length limit; use "Submit to GitHub" for full log)',
            '',
            '---',
            'Please attach a screenshot if needed (use PrtScn or snipping tool).',
            'Auto-generated by BIM OOTB bug reporter',
          ].join('\n');
          return 'mailto:red1org@gmail.com?subject=' +
            encodeURIComponent(subject) + '&body=' + encodeURIComponent(body);
        };

        // §BUGREPORT_MAILTO_LEN — mailto: is handed to the OS's registered mail
        // client, whose length ceiling (~2000 chars on Windows/Outlook) is far
        // stricter than a browser URL bar; past it the OS silently drops the
        // launch with no JS error. Cascade log lines down until the URL fits —
        // same idea as the GitHub truncation below, tighter budget.
        var emailUrl;
        var counts = [50, 20, 10, 5, 0];
        for (var ci = 0; ci < counts.length; ci++) {
          emailUrl = buildEmailUrl(counts[ci]);
          if (emailUrl.length <= 1800) break;
        }
