...
const CACHE_VERSION = 'v602';   // v602: pills_idmp.json gets the same 'compare' pill (iDempiere surface) + idmp_pills.js learns external nav — bumped so the precached pills_idmp.json refreshes. v601: pills.json adds the 'compare' pill (Migrate & Compare → BIMCompiler docs MigrateComparisonPaper, new tab) — bumped so the precached pills.json refreshes. Prior: v600 §INTEG-WIRE — period-close in-app (erp_period_close.js + period_close_ui.js): an accountant closes a period → signed checkpoint = balance b/f on the live sidecar op-log; next load bootstraps from the checkpoint. Substrate (prompts/ERP_SUBSTRATE_INTEGRATION.md Phase 2 slice A) on the collapsed canonical kernel (commitGroup). Witness erp/tests/poc_period_close_wire.js (§INTEG-WIRE).
