...
const CACHE_VERSION = 'v607';   // v607: §INTEG-WIRE-B — disposable-host persistence in-app (erp_replica_client.js + erp_persist_ui.js): back the books up to a SIGNED snapshot the user owns (signed by the edge key), restore on a FRESH device → replay recomputes tip == signed tip + books to the cent; a tampered/forged file is rejected (no private key). Substrate prompts/ERP_SUBSTRATE_INTEGRATION.md Phase 2 slice B; witness erp/tests/poc_persist_wire.js (§INTEG-WIRE-B). v606: PILL-ORGANISE — Read/Compare lightbulb pill (id=erpdoc) + Install/Migrate icons download/arrowRightLeft + erp.html stray compare-link removed (#206). v605: §0.10a front-door onboarding — Install + Migrate first-class buttons on the iDempiere LOGIN CARD (via ErpPicker); in-client the rail pills DEMOTE to the ⋯ overflow (idmp_pills.js _applyStage). v604: migrate_compare.html now FORWARDS to BIMCompiler docs MigrateComparisonPaper (single source, no local-copy drift); the erpdoc 'Read ERP' paper pill gets a distinct 'lightbulb' icon (was 'doc', looked like the other doc pills). v603: the existing 'erpdoc' (Read ERP) pill now links to the Migrate & Compare paper (was BIMCompiler/ERP/) — a one-line URL change; ALSO reverted the erroneously-added 'compare' pill on both manifests + the idmp_pills.js nav block (#198/#199 are undone, net = just the erpdoc link). Prior v601/v602 were that erroneous add. Prior: v600 §INTEG-WIRE — period-close in-app (erp_period_close.js + period_close_ui.js): an accountant closes a period → signed checkpoint = balance b/f on the live sidecar op-log; next load bootstraps from the checkpoint. Substrate (prompts/ERP_SUBSTRATE_INTEGRATION.md Phase 2 slice A) on the collapsed canonical kernel (commitGroup). Witness erp/tests/poc_period_close_wire.js (§INTEG-WIRE).
