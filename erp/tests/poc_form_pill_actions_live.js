...
(async () => {
  ...
  // (4) click New → P3: New is IN PLACE (inline create, no modal, no ring). doctrine §0: the ring is Glass-only.
  logs = [];
  ...
  //   correctly NOT a permitted verb, so the inline create declines (§INPLACE-NEW … skipped). The regression that
  //   matters: New routes to the inline create seam (§FORM-PILL new=inline-create), NEVER the ring fan, ring CLOSED.
  const newLog = [...logs].reverse().find(l => l.startsWith('§FORM-PILL new=inline-create')) || '(none)';
  const skipLog = [...logs].reverse().find(l => /§INPLACE-NEW table=c_invoice/.test(l)) || '(none)';
  const ringOpen = await page.evaluate(() => { const r = document.getElementById('crudRing'); return !!(r && r.classList.contains('open')); });
  const modal = await page.evaluate(() => !!document.querySelector('#crudForm.open'));
  console.log('§FORM-PILL ' + newLog + ' inlinenew="' + skipLog + '" ringOpen=' + ringOpen + ' modal=' + modal);
  ok('New pill routes to the inline create seam, NOT the ring fan / modal (§FORM-PILL new=inline-create, ring+modal closed)',
     /§FORM-PILL new=inline-create/.test(newLog) && /§INPLACE-NEW table=c_invoice/.test(skipLog) && !ringOpen && !modal, newLog + ' ring=' + ringOpen);
