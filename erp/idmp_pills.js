...
(function () {
  ...
  function _worldDrawer(srcBtn) {
    ...
    // PERPENDICULAR to the iDempiere VERTICAL right-edge pill strip → the drawer extends SIDEWAYS (a row to
    // the LEFT of the W pill), not a column above it (the upward column ran PARALLEL to the strip → it
    // COVERED the pills stacked above W). Mirrors the viewer fix (panels.js _worldHistDrawer, erp/viewer
    // sw v651). Row order: bomb first (leftmost = outside/far, safety), Z last (rightmost = adjacent to the
    // pill, first reach).
